...
class DINOv(nn.Module):
    ...
    def forward(self, batched_inputs, inference_task='seg', dataset_name=''):
        ...
        if self.training:
            ...
            if self.task_switch['coco']:
                ...
                # 如果提供了'sam'数据，它会优先加载；如果只提供'coco'，也能正常工作；
                if isinstance(batched_inputs, list):
                    data = batched_inputs
                elif 'coco' in batched_inputs:
                    data = batched_inputs['coco']
                elif 'sam' in batched_inputs:
                    data = batched_inputs['sam']
                else:
                    raise KeyError("No valid data key found in batched_inputs. Expected one of ['coco', 'sam']")
    ...
    def forward_seg(self, batched_inputs, task='seg', prediction_switch={'part': True, 'whole': True, 'seg': False, 'det': True}, coco_track=False, dataset_name=''):
        """
        Args:
        ...
        if self.training:
            ...
            prediction_switch = {'part': False, 'whole': False, 'seg': False, 'det': True}
    ...
    def evaluate_demo_content_openset_multi_with_content_features(self, batched_inputs, mask_features, multi_scale_features,
        ...
        return src_boxes, pred_masks, pred_ious, ori_masks, scores_per_image_openset
