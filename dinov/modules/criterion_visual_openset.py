...
class SetCriterionVisualOpenSet(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, layer_id=None, extra=None):
        ...
        print("\n========== Debug: loss_masks ==========")
        NUM_CLASSES = 133  # 你可以根据 config 动态设定

        for i, target in enumerate(targets):
            labels = target.get("labels", None)
            if labels is None:
                print(f"[Target {i}] ❌ Missing 'labels'")
                continue

            if labels.numel() == 0:
                print(f"[Target {i}] ⚠️ Empty 'labels'")
                continue

            # 清洗非法 label 值
            if labels.max() >= NUM_CLASSES or labels.min() < 0:
                print(f"[Target {i}] 🚨 Label out of range: {labels.min().item()} ~ {labels.max().item()}, clamping")
        ...
        assert "pred_masks" in outputs, "❌ 'pred_masks' not in outputs"
        if indices is None or len(targets) == 0 or targets[0].get('fake', False):
            print("⚠️ Skipping loss due to empty targets or fake data")
            loss = outputs['pred_masks'].sum() * 0.0
            return {"loss_mask_bce_0": loss, "loss_mask_dice_0": loss}

        # 🧪 Debug: targets labels
        for i, target in enumerate(targets):

            if "labels" not in target:
                print(f"[Target {i}] ❌ Missing 'labels'")
            elif target["labels"].numel() == 0:
                print(f"[Target {i}] ⚠️ Empty 'labels'")
            else:
                max_label = target["labels"].max().item()
                min_label = target["labels"].min().item()
                print(f"[Target {i}] ✅ Labels range: {min_label} ~ {max_label}")
                assert max_label < 133, f"❗️ Label {max_label} >= NUM_CLASSES (133)"

        # 🧪 Debug: indices
        for i, (src, tgt) in enumerate(indices):

            print(f"[Match {i}] src len: {len(src)}, tgt len: {len(tgt)}")
            if src.numel() == 0 or tgt.numel() == 0:
                print(f"[Match {i}] ⚠️ Empty match pair")
        ...
        src_masks = outputs["pred_masks"]
        print("src_masks.shape (before index):", src_masks.shape)


        src_masks = src_masks[src_idx]
        print("src_masks.shape (after index):", src_masks.shape)
        ...
        target_masks, valid = nested_tensor_from_tensor_list(masks).decompose()
        target_masks = target_masks.to(src_masks.device)
        print("target_masks.shape (before index):", target_masks.shape)

        target_masks = target_masks[tgt_idx]
        print("target_masks.shape (after index):", target_masks.shape)


        # Format for point sampling
        src_masks = src_masks[:, None]
        target_masks = target_masks[:, None]

        if torch.isnan(src_masks).any() or torch.isinf(src_masks).any():
            print("⚠️ 模型输入中有 NaN 或 Inf，跳过此 batch")
            return {}  # 或者返回无损失

        # Check src_idx values
        print("src_idx:", src_idx)
        print("src_masks.shape (original):", outputs["pred_masks"].shape)

        # Explicit check
        src_mask_shape = outputs["pred_masks"].shape
        max_idx = src_idx[0].max().item() if len(src_idx[0]) > 0 else -1
        if max_idx >= src_mask_shape[0]:
            raise ValueError(f"❌ src_idx out of range: max {max_idx} >= {src_mask_shape[0]}")

        # 🧪 Check for NaNs / Infs
        if torch.isnan(src_masks).any() or torch.isinf(src_masks).any():
            raise ValueError("❌ src_masks contains NaN or Inf")

        if torch.isnan(target_masks).any() or torch.isinf(target_masks).any():
            raise ValueError("❌ target_masks contains NaN or Inf")

        # 🔥 This is the actual place that triggers crash
        with torch.no_grad():
            point_coords = get_uncertain_point_coords_with_randomness(
                src_masks.type(self.empty_weight.dtype),
                lambda logits: calculate_uncertainty(logits.type(self.empty_weight.dtype)),
